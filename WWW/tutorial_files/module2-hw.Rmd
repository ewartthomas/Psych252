...
(First we load `{ggplot}`; often, it's good practice to load in ALL the packages you'll use in the homework at the very beginning, and that way you won't forget and load in the same package twice!)
```{r}
library(ggplot2)
```
...
What's happening is that the "aesthetic" object in `{ggplot()`} doesn't understand that this variable should really be a factor when you try to plot it as a boxplot -- that's because `RTWL` is actually an integer! We can see that when we call `str()` on the dataframe:
```{r}
str(df_RTWL$RTWL)
```

Note that geom_point doesn't have this problem:
...
Make sure you **double check your dataframe** after you've loaded it in, and check to see the **structure of each of your columns**! Confusion between integers and factors can often be an issue, especially if your subjects are coded as numbers!!
...
Here, the difference is not quite significant! So it's important you tell R that the groups are paired when the two samples are dependent on one another! Now, before you finish this hw example, **write up the results of these last two statistical tests and interpret your results in the larger context of right to work laws**:
