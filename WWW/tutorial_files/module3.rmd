...
We also need to rescale `id` to 1:10 within each level of `attn`, since the subject id (`id`) is 11:20 when attn is 'focused'. So we need to select only these values of `id`, and transform them to 1:10. This requires creating a new column, `cond.id`, in the dataframe, `dl`.
