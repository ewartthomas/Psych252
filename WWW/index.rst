Statistical Methods for Behavioral and Social Sciences
======================================================

.. image:: images/pic1.png
   :width: 30%
   :alt: alternate text
   :target: https://gallery.shinyapps.io/simple_regression/
.. image:: images/pic2.png
   :width: 30%
   :alt: alternate text
   :target: https://gallery.shinyapps.io/sampling_and_stderr/
.. image:: images/pic3.png
   :width: 30%
   :alt: alternate text
   :target: https://supsych.shinyapps.io/logistic_regression

Instructors:	  	  
~~~~~~~~~~~~

Ewart Thomas, Benoît Monin

2015 Teaching Assistants:
~~~~~~~~~~~~~~~~~~~~

Dan Birman, Steph Gagnon, Robert Hawkins

Contents
--------

.. toctree::
   :maxdepth: 1

   data/index.rst
   slides/index.rst
   plots/index.rst
   apps/index.rst
   tutorials/index.rst
   cheatsheets/index.rst
   section/index.rst
   section_2013/index.rst
   
