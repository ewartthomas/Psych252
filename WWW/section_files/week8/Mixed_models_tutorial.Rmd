...
# Compare lm with nlme model to use same log likelihood
library(nlme)
res5c = lme(pitch ~ condition + gender, random=~1|subject, data = na.omit(d))
...
Here, the $\chi^2$ distribution isn't always a good approximation of the null distribution, so keep that in mind. You can use a parametric bootstrap to get a better p-value, but we won't cover that here.
