...
$$\text{pitch} ~ \text{politeness} + \text{sex} + (1 | \text{subject}) \epsilon$$
...
$$\text{pitch} = \text{intercept_bysubject} + \text{condition} + \text{gender}$$
...
On the other hand, some argue that model comparison is a better way of testing whether a parameter is signifiant. That is, if adding the parameter to your model significantly improves model fit, then that parameter should be included in the model.
...
```{r}
res5 = lmer(pitch ~ condition + gender + (1 | subject), REML = FALSE, data = d)
