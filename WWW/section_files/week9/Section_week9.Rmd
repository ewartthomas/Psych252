...
Now we'll try working with the data from homework 5 (that was also used in the tutorial in Week 0 if you were there!) to get used to using mixed models.
...
Since the levels of 'num' were created from the original column names (i.e., num1, num2, num3), R interprets the 'num' variable as a factor. However, if we want to treat num as a quantitative variable, we need to force num to be numeric. We also want the subject id ('id') to be a factor:
...
We also need to change `id` to be 1:10 within each level of `attn`, since the subject id ('id') is coded 11:20 when `attn` is 'focused', even though the same subject completed both task (i.e., subject 1 = 11, subject 2=12). So we need to select only these values of 'id', and transform them to 1:10. This requires creating a new variable, cond.id, in the dataframe, `dl`.
...
The correlation between our random intercepts and slopes for `num` is -1 -- as our intercepts get higher, the slope gets smaller. This is a little suspicious...
