...
## Compare to aov
```{r}
anova(lmer(Recall ~ Task * Valence + (1|Subject), data=d))
summary(aov(Recall~(Task*Valence)+Error(Subject),d)) # same as lmer w/random intercepts
summary(aov(Recall~(Task*Valence)+Error(Subject/(Task*Valence)),d))
```
